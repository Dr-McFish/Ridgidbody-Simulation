...
int main() {
	...
	initialize_rigidbody(test_bodyl, 10, Eigen::Matrix3f::Identity(), Eigen::Vector3f(0, 1, 0));
	struct rendering::mesh test_cube = rendering::create_cube("test_cube");	

	// if desired, set up a userCallback to add ImGui UI elements

	while(true) {
		full_integration_step1(test_bodyl, 1./620.);
